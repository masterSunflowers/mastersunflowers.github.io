...
$$v(s) = E[G_t | S_t=s] = E[R_{t+1} + R_{T+2} + R_{t+3} + ... | S_t = s, A_t = a]$$
...
Với luật ngẫu nhiên (stochastic), dựa theo xác suất: $\pi(A|S) = p(A|S)$

3. **Value Function**
$$v_\pi(s) = E[G_t | S_t = s, \pi] = E[R_{t+1} + \gamma R_{t+2} + \gamma^2 R_{t+3} + ... | S_t=s, \pi]$$
...
tương tự ta có:
$$v_\pi(s) = E[R_{t+1} + \gamma G_{t+1} | S_t = s, A_{t} \sim \pi(s)] = E[R_{t+1} + \gamma v_\pi(S_{t+1}) | S_t = s, A_{t} \sim \pi(s)]$$
...
Ví dụ ta có 1 agent giúp ta đánh bạc hiệu quả trong các Casino, agent này có thể chọn các máy chơi từ 1->n các máy này có tỉ lệ đánh thắng là {$R_a | a \in A$}
Trong đó:
...
- Giá trị hành động của hành động a (lựa chọn máy a để chơi) có phần thưởng kỳ vọng (tỷ lệ thắng) là: $$q(a) = E[R_t | A_t=a]$$
- Giá trị tối ưu là: $$v_* = \max_{a \in A}q(a) = \max_aE[R_t | A_t = a]$$
